...
public partial class ParkingService : IParkingService
{
        ...
        Guid providerUserId, Guid facilityId, CancellationToken ct = default)
    {
        ...
        // A property the admin is holding or has published is the admin's to withdraw.
        if (facility.Status is not (ParkingStatus.DRAFT or ParkingStatus.REJECTED))
        {
            throw new InvalidOperationException(
                $"Only a draft or rejected property can be deleted. This one is {facility.Status}.");
        }

        var assets = facility.Documents.Select(d => d.PublicId).ToList();

        // Bays, allocations, documents and section reviews all cascade off the property.
        _context.Set<ParkingFacility>().Remove(facility);
        await _context.SaveChangesAsync(ct);

        foreach (var publicId in assets)
        {
            await DestroyAssetAsync(publicId, ct);
        }
    }
    ...
    // One of the owner's own properties, in any state. Ownership and 404 come from the shared gate.
    public async Task<ParkingResponse> GetProviderFacilityAsync(
        Guid providerUserId, Guid facilityId, CancellationToken ct = default)
    {
        var facility = await GetOwnedFacilityAsync(providerUserId, facilityId, ct);
        ...
        ParkingStatus? status, string? provider, CancellationToken ct = default)
    {
        ...
        if (owner is not null)
        {
            ...
            query = query.Where(f =>
                EF.Functions.ILike(f.Provider!.User!.FullName, pattern) ||
                EF.Functions.ILike(f.Provider!.User!.Email, pattern) ||
                EF.Functions.ILike(f.Provider!.BusinessName!, pattern));
        }
        ...
        return facilities.Select(f => new FacilityQueueRowResponse
        {
            ...
            ProviderName = f.Provider?.User?.FullName ?? string.Empty,
            ProviderEmail = f.Provider?.User?.Email ?? string.Empty,
            ProviderBusinessName = f.Provider?.BusinessName,
        ...
        Guid providerUserId, CancellationToken ct = default)
    {
        ...
        // No denormalized fields to sync — provider info is accessed through navigation properties.
        return new ProviderIdentitySyncResponse
        {
            ProviderUserId = providerUserId,
            ProviderName = provider.User?.FullName ?? string.Empty,
            ProviderEmail = provider.User?.Email ?? string.Empty,
