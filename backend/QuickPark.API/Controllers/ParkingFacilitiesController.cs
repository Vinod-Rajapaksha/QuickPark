...
public class ParkingFacilitiesController : ControllerBase
{
    ...
    // One of the caller's own properties, whatever its approval state.
    [HttpGet("me/{id:guid}")]
    [Authorize(Roles = OwnerRole)]
    public async Task<IActionResult> GetMyFacility(Guid id, CancellationToken ct)
    {
        if (!this.TryGetUserId(out var userId)) return Unauthorized();

        try
        {
            var facility = await _parkingService.GetProviderFacilityAsync(userId, id, ct);
            return Ok(facility);
        }
    ...
    public async Task<IActionResult> Delete(Guid id, CancellationToken ct)
    {
        ...
        try
        {
            await _parkingService.DeleteFacilityAsync(userId, id, ct);
            return NoContent();
        }
