...
export const parkingApi = {
  ...
  deleteFacility: async (facilityId: string): Promise<void> => {
    await axiosClient.delete(`${BASE}/${facilityId}`);
  },
  ...
  // Owner-scoped: returns the property in any state, unlike the approved-only read above.
  getOwnerFacility: async (facilityId: string): Promise<ParkingFacility> => {
    const response = await axiosClient.get(`${BASE}/me/${facilityId}`);
    return response.data;
  },
